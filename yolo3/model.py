...
'''
如果使用的是yolo_tiny.weight，则需要以下代码：
...
def yolo_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape): # 返回预测框的左下角与右上角的坐标与得分
...
'''
函数解析：
    yolo_outputs：模型输出，格式如下[（?，13,13,255）（？，26,26,255）（?,52,52,255）] 
...
'''
这一部分的逻辑：
    在yolo_eval()函数中调用yolo_boxes_and_scores()得到目标框、目标框得分和类别。
    而在yolo_boxes_and_scores()函数中，先调用yolo_head()函数计算每一个网格的目标对象的中心点坐标box_xy和目标框的宽与高box_wh，
    以及目标框的置信度box_confidence和类别置信度box_class_probs；
    然后调用yolo_correct_boxes（），将box_xy, box_wh转换为输入图片上的真实坐标，输出boxes是框的左下、右上两个坐标(y_min, x_min, y_max, x_max)
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes):
    ...
    ##掩膜尺寸:[图片张数，每张图box个数] 即每个box都有一个0或1的掩膜来掩w宽
    ...
    for b in range(m):
        ...
        #这是一个神奇的掩膜操作，可以符合条件的wh直接筛选出来，尺寸为(每张图掩膜以后的box个数,2)
        ...
        for t, n in enumerate(best_anchor):
            for l in range(3):
                if n in anchor_mask[l]: # 看一下best_anchor在哪个尺度
                    ...
                    n = anchor_mask[l].index(n) # 应该放在该尺度该grid三个anchor的哪个位置上
                    c = true_boxes[b,t, 4].astype('int32')  # 类别获取
                    y_true[l][b, j, i, n, 0:4] = true_boxes[b,t, 0:4]
                    y_true[l][b, j, i, n, 4] = 1
                    y_true[l][b, j, i, n, 5+c] = 1
                    # 将分类器的结果变成0和1的形式，分到的那类是1
                    break
